...
class Process:
    def __init__(self, dataset):
        ...
        self.mysqlHelper = MySQLHelper("localhost", "root", "root")
    ...
    def handle(self, sourceKey, sourceStr, targetDic):
        ...
        whereData = {"source":sourceKey, "dataset":self.dataset}
        self.mysqlHelper.delete("text_similarity", whereData)
        ...
        for key in targetDic:
            ...
            # 此处应该写数据库， 保存S和T的相似度
            dataSource = {"source": sourceKey, "target": key, "similarity": str(sim),
                          "jaccard": str(jaccard), "dataset": self.dataset}
            self.mysqlHelper.insert("text_similarity", dataSource)
